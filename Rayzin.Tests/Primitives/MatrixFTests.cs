...
namespace Rayzin.Tests.Primitives
{
    ...
    public class MatrixFTests
    {
        ...
        {
            var a = new MatrixF(4, input);
            var output = a.Inverse();

            Assert.That(output, Is.EqualTo(new MatrixF(4, expected)));
        }

        [Test]
        public void MultiplyResultOfMultiplyingAByBWithTheInverseOfBToGetBackToA()
        {
            var a = new MatrixF(4, 3, -9, 7, 3, 3, -8, 2, -9, -4, 4, 4, 1, -6, 5, -1, 1);
            var b = new MatrixF(4, 8, 2, 2, 2, 3, -1, 7, 0, 7, 0, 5, 4, 6, -2, 0, 5);
            var c = a * b;
